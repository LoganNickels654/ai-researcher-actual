...
# Supabase setup - Two clients for different purposes
SUPABASE_URL = os.getenv("SUPABASE_URL")
SUPABASE_ANON_KEY = os.getenv("SUPABASE_ANON_KEY")
SUPABASE_SERVICE_KEY = os.getenv("SUPABASE_SERVICE_KEY")

# Client for user authentication and user-facing operations
...
async def verify_user_and_check_limits(authorization: str = Header(None)):
    ...
    try:
        # Verify the user's token with Supabase (using regular client for auth)
...
async def get_user_limits(user_id: str) -> UserLimits:
    ...
    """
    try:
        # Get user's subscription info using admin client
        ...
        usage_result = supabase_admin.table("user_usage").select("*").eq("user_id", user_id).eq("date", str(today)).execute()
...
async def increment_user_usage(user_id: str):
    ...
    try:
        ...
        # Check if user already has a record for today using admin client
        existing = supabase_admin.table("user_usage").select("*").eq("user_id", user_id).eq("date", str(today)).execute()
        
        if existing.data and len(existing.data) > 0:
            # Update existing record using admin client
            new_count = existing.data[0]["searches_count"] + 1
            supabase_admin.table("user_usage").update({"searches_count": new_count}).eq("user_id", user_id).eq("date", str(today)).execute()
        else:
            # Create new record for today using admin client
...
async def save_paper(
    ...
    try:
        # Use admin client for saving papers (system operation)
        result = supabase_admin.table("saved_papers").insert({
            "user_id": user.id,
            "title": paper.title,
            "authors": paper.authors,
            "abstract": paper.abstract,
...
async def get_saved_papers(user_data = Depends(verify_user_and_check_limits)):
    ...
    try:
        # Use admin client to fetch saved papers
        result = supabase_admin.table("saved_papers").select("*").eq("user_id", user.id).execute()
        return result.data
...
async def export_papers(user_data = Depends(verify_user_and_check_limits)):
    ...
    try:
        # Use admin client to fetch papers for export
        result = supabase_admin.table("saved_papers").select("*").eq("user_id", user.id).execute()
